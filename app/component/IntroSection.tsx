...
export const IntroSection = () => {
  ...
  return (
    ...
    >
      ...
      >
        <h1 className={`text-6xl lg:text-[150px] ${theme === "crazy" ? "text-summerRed": "text-customGrey"} mb-16`}>
          Welcome
        </h1>
        <h2 className={`text-4xl lg:text-[60px] ${theme === "crazy" ? "text-summerRed": "text-customGrey"}`}>
          I&apos;m Spencer,
          <br />
          <br />
          Software Developer{" "}
        ...
        <p className={`text-xl lg:text-2xl text-customGrey text-center md:text-start border-l-2  px-4 lg:max-w-3xl lg:mt-12 ${
          theme === "aqua" ? "border-aqua" : theme === "cyber" ? "border-elegantPink" : theme === "crazy" ? "border-summerPink text-summerRed": "border-aqua"}`}>
      ...
      <ChatButton />
      <Image
        className={`z-10 absolute top-0 lg:top-1/2 lg:left-32 lg:transform lg:-translate-x-1/2 lg:-translate-y-1/2 opacity-20 ${theme === "crazy" ? "lg:opacity-30" : "lg:opacity-60"} rounded-4xl bg-opacity-0/`}
        width={900}
        src={me}
      ...
      <div
        className={`hidden lg:flex z-10 h-[800px] w-[800px] absolute top-0 -right-96 rounded-full ${
          theme === "aqua" ? "glowing-shadow" : theme === "cyber" ? "cyber-shadow" : theme === "crazy" ? "crazy-shadow bg-summerYellow bg-opacity-50": "glowing-shadow"}`}
      ></div>
    </motion.div>
  );
};
